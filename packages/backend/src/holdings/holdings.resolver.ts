...
export class HoldingsResolver {
  ...
  @Query(() => [String])
  async tagsForHolding(@Args('holdingSymbol') holdingSymbol: string): Promise<string[]> {
  ...
  @Mutation(() => HoldingTag)
  async addHoldingTag(@Args('input') input: AddHoldingTagInput): Promise<HoldingTag> {
    return this.holdingsService.addTag(input);
  }

  @Mutation(() => Boolean)
  async removeHoldingTag(@Args('input') input: RemoveHoldingTagInput): Promise<boolean> {
    return this.holdingsService.removeTag(input);
  }

  @Mutation(() => [HoldingTag])
  async setHoldingTags(@Args('input') input: SetHoldingTagsInput): Promise<HoldingTag[]> {
