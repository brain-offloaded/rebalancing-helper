...
interface RebalancingGroupDetailPageProps {
  groupId: string;
  onClose: () => void;
}
...
export const RebalancingGroupDetailPage: React.FC<
  RebalancingGroupDetailPageProps
> = ({ groupId, onClose }) => {
  const {
    data: groupsData,
  ...
  if (!group) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>선택한 리밸런싱 그룹을 찾을 수 없습니다.</CardTitle>
        </CardHeader>
                        ...
                        <Td>
                          <TextInput
                            ...
                            onChange={(event) =>
                              setTargetAllocations((prev) => ({
                                ...prev,
                                [tagId]: Number(event.target.value) || 0,
                              }))
                            }
                          />
                        </Td>
